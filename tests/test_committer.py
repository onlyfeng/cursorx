...
class TestCheckStatus:
    ...
    def test_clean_repo_returns_no_changes(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试干净仓库返回空更改列表"""
    ...
    def test_modified_file_detected(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试修改文件时返回正确的文件列表"""
        # 修改已有文件
    ...
    def test_new_file_detected(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试新文件被检测为未跟踪"""
        # 创建新文件
    ...
    def test_staged_file_detected(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试已暂存的文件"""
        # 创建并暂存新文件
...
class TestGenerateCommitMessage:
    ...
    @pytest.mark.asyncio
    async def test_generate_message_from_diff(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试根据 diff 生成合理的提交信息"""
        # 修改文件创建 diff
        readme = temp_git_repo / "README.md"
        readme.write_text("# Updated Test Repo\n\nNew feature added.\n")
        ...
        with patch.object(committer.cursor_client, "execute", new=AsyncMock(return_value=mock_result)):
            message = await committer.generate_commit_message()

        assert message == "feat(docs): 更新 README 文档"

    @pytest.mark.asyncio
    async def test_generate_message_with_provided_diff(self, committer: CommitterAgent):
        """测试使用提供的 diff 生成提交信息"""
        diff = """
...
"""
    ...
    @pytest.mark.asyncio
    async def test_fallback_message_when_cursor_fails(self, committer: CommitterAgent):
    ...
    @pytest.mark.asyncio
    async def test_fallback_message_simple_style(self, temp_git_repo: Path):
        """测试 simple 风格的默认提交信息"""
        config = CommitterConfig(
            working_directory=str(temp_git_repo),
            commit_message_style="simple",
        ...
        with patch.object(agent.cursor_client, "execute", new=AsyncMock(return_value=mock_result)):
            message = await agent.generate_commit_message(diff="some diff")

        assert message == "更新"

    @pytest.mark.asyncio
    async def test_empty_diff_returns_default(self, committer: CommitterAgent):
        """测试空 diff 返回默认提交信息"""
        # 不 mock cursor_client，直接测试空 diff 路径
        with patch.object(committer, "_get_diff", return_value=""):
            message = await committer.generate_commit_message()
...
class TestCommit:
    ...
    def test_commit_success(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试正常提交流程"""
        # 创建新文件
    ...
    def test_commit_multiple_files(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试提交多个文件"""
        # 创建多个文件
    ...
    def test_commit_specific_files(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试只提交指定文件"""
        # 创建多个文件
    ...
    def test_commit_with_exclude_pattern(self, temp_git_repo: Path):
        """测试排除文件模式"""
        config = CommitterConfig(
            working_directory=str(temp_git_repo),
    ...
    def test_commit_no_changes(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试无变更时提交失败"""
    ...
    def test_commit_all_excluded(self, temp_git_repo: Path):
        """测试所有文件都被排除时提交失败"""
        config = CommitterConfig(
            working_directory=str(temp_git_repo),
            exclude_files=["*.txt"],
...
class TestRollback:
    ...
    def test_rollback_soft(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试软回退（保留工作区和暂存区）"""
        # 创建并提交一个文件
    ...
    def test_rollback_mixed(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试混合回退（保留工作区，清空暂存区）"""
        # 创建并提交一个文件
    ...
    def test_rollback_hard(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试硬回退（清空工作区和暂存区）"""
        # 创建并提交一个文件
    ...
    def test_rollback_checkout_files(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试还原指定文件"""
        # 修改已有文件
        readme = temp_git_repo / "README.md"
    ...
    def test_rollback_invalid_mode(self, committer: CommitterAgent):
        """测试无效的回退模式"""
    ...
    def test_rollback_to_specific_commit(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试回退到指定提交"""
        # 创建多个提交
...
class TestCommitIteration:
    ...
    @pytest.mark.asyncio
    async def test_commit_iteration_success(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试正常迭代提交"""
        # 创建新文件
    ...
    @pytest.mark.asyncio
    async def test_commit_iteration_with_auto_push(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试带自动推送的迭代提交"""
        # 创建新文件
        ...
        with (
            patch.object(committer.cursor_client, "execute", new=AsyncMock(return_value=mock_result)),
            patch.object(committer, "push", return_value=mock_push_result),
        ):
            result = await committer.execute("提交并推送", context={"auto_push": True})

        assert result["success"] is True
        assert result["pushed"] is True

    @pytest.mark.asyncio
    async def test_commit_iteration_no_changes(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试无变更时的处理"""
        # 不创建任何新文件，仓库是干净的
...
class TestGetCommitSummary:
    ...
    def test_get_commit_summary_empty(self, committer: CommitterAgent):
        """测试无提交时的摘要"""
    ...
    def test_get_commit_summary_after_commits(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试多次提交后的统计"""
        # 执行多次提交
    ...
    def test_get_commit_summary_with_failures(self, committer: CommitterAgent, temp_git_repo: Path):
        """测试包含失败提交的统计"""
        # 成功提交
