...
class TestRunBasicMpStreamJsonConfig:
    ...
    def test_stream_json_config_default_values(
...
class TestFindConfigFile:
    ...
    def test_find_config_file_from_cwd(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
    ...
    def test_find_config_file_from_project_root(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
    ...
    def test_find_config_file_cwd_priority_over_parent(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
...
class TestIndexingCliLoadConfig:
    ...
    def test_load_config_auto_discover_from_cwd(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
...
class TestDefaultConstantsConsistency:
    ...
    def test_default_max_iterations_matches_config_yaml(
        self, project_config_yaml: dict
    ) -> None:
        ...
        if yaml_value is not None:
            assert DEFAULT_MAX_ITERATIONS == yaml_value, (
                f"DEFAULT_MAX_ITERATIONS ({DEFAULT_MAX_ITERATIONS}) "
                f"与 config.yaml ({yaml_value}) 不一致"
            )

    def test_default_worker_pool_size_matches_config_yaml(
        self, project_config_yaml: dict
    ) -> None:
        ...
        if yaml_value is not None:
            assert DEFAULT_WORKER_POOL_SIZE == yaml_value, (
                f"DEFAULT_WORKER_POOL_SIZE ({DEFAULT_WORKER_POOL_SIZE}) "
                f"与 config.yaml ({yaml_value}) 不一致"
            )

    def test_default_models_match_config_yaml(
        self, project_config_yaml: dict
    ) -> None:
        ...
        if yaml_planner is not None:
            assert DEFAULT_PLANNER_MODEL == yaml_planner, (
                f"DEFAULT_PLANNER_MODEL ({DEFAULT_PLANNER_MODEL}) "
                f"与 config.yaml ({yaml_planner}) 不一致"
            )

        yaml_worker = models_config.get("worker")
        if yaml_worker is not None:
            assert DEFAULT_WORKER_MODEL == yaml_worker, (
                f"DEFAULT_WORKER_MODEL ({DEFAULT_WORKER_MODEL}) "
                f"与 config.yaml ({yaml_worker}) 不一致"
            )

        yaml_reviewer = models_config.get("reviewer")
        if yaml_reviewer is not None:
            assert DEFAULT_REVIEWER_MODEL == yaml_reviewer, (
                f"DEFAULT_REVIEWER_MODEL ({DEFAULT_REVIEWER_MODEL}) "
                f"与 config.yaml ({yaml_reviewer}) 不一致"
            )

    def test_default_cloud_timeout_matches_config_yaml(
        self, project_config_yaml: dict
    ) -> None:
        ...
        if yaml_value is not None:
            assert DEFAULT_CLOUD_TIMEOUT == yaml_value, (
                f"DEFAULT_CLOUD_TIMEOUT ({DEFAULT_CLOUD_TIMEOUT}) "
                f"与 config.yaml ({yaml_value}) 不一致"
            )

    def test_default_cloud_auth_timeout_matches_config_yaml(
        self, project_config_yaml: dict
    ) -> None:
        ...
        if yaml_value is not None:
            assert DEFAULT_CLOUD_AUTH_TIMEOUT == yaml_value, (
                f"DEFAULT_CLOUD_AUTH_TIMEOUT ({DEFAULT_CLOUD_AUTH_TIMEOUT}) "
                f"与 config.yaml ({yaml_value}) 不一致"
            )
...
class TestTableDrivenConfigFields:
    ...
    def test_config_field_loading(
        ...
        assert actual_value == expected_value, (
            f"字段 {field_path}: 期望 {expected_value}, 实际 {actual_value}"
        )
    ...
    def test_default_values_when_no_config(
        ...
        with patch.object(ConfigManager, "_find_config_file", return_value=None):
            ...
            assert actual_value == expected_value, (
                f"字段 {field_path}: 期望 DEFAULT 值 {expected_value}, "
                f"实际 {actual_value}"
            )
...
class TestOrchestratorConfigTriStateModel:
    ...
    def test_orchestrator_resolves_models_from_yaml_when_none(
        ...
        """
        from coordinator.orchestrator import OrchestratorConfig, Orchestrator
        ...
        with patch("coordinator.orchestrator.PlannerAgent") as mock_planner, \
...
class TestResolveOrchestratorSettings:
    ...
    def test_resolve_orchestrator_settings_execution_mode_forces_basic(
        ...
        settings_auto = resolve_orchestrator_settings(overrides={
            "execution_mode": "auto",
            "orchestrator": "mp",
        })

        assert settings_auto["orchestrator"] == "basic", (
            "execution_mode=auto 时应强制使用 basic 编排器"
        )
...
class TestRunIterateGetOrchestratorType:
    ...
    def test_get_orchestrator_type_cloud_forces_basic(
        self, base_iterate_args: argparse.Namespace
    ) -> None:
    ...
    def test_get_orchestrator_type_auto_forces_basic(
        self, base_iterate_args: argparse.Namespace
    ) -> None:
    ...
    def test_get_orchestrator_type_cli_allows_mp(
        self, base_iterate_args: argparse.Namespace
    ) -> None:
        """测试 execution_mode=cli 时允许使用 mp 编排器"""
        from scripts.run_iterate import SelfIterator
...
class TestCloudClientConfigPriority:
    ...
    def test_build_cloud_client_config_overrides_highest_priority(
        ...
        config = build_cloud_client_config(overrides={
...
class TestCloudAuthManagerApiKeyPriority:
    ...
    def test_cloud_auth_manager_env_cursor_cloud_api_key(
        self, monkeypatch
    ) -> None:
    ...
    def test_cloud_auth_manager_cursor_api_key_over_cloud_api_key(
        self, monkeypatch
    ) -> None:
    ...
    def test_cloud_auth_manager_config_api_key_highest_priority(
        self, monkeypatch
    ) -> None:
...
class TestRunMpPyTriStateOptions:
    ...
    def test_mp_parse_args_strict_review_default_none(
        ...
        assert args.strict_review is None, (
            "不传 --strict 或 --no-strict 时，strict_review 应为 None"
        )
    ...
    def test_mp_run_orchestrator_uses_config_defaults_when_cli_none(
        ...
        assert captured_config["strict_review"] is True, (
            "CLI 未指定时，strict_review 应使用 config.yaml 的值 (True)"
        )
    ...
    def test_mp_run_orchestrator_cli_overrides_config(
        ...
        assert captured_config["enable_sub_planners"] is True, (
            "CLI 显式指定 True 应覆盖 config.yaml 的 False"
        )
        assert captured_config["strict_review"] is False, (
            "CLI 显式指定 False 应覆盖 config.yaml 的 True"
        )
...
class TestOrchestratorModelFromConfigYaml:
    ...
    def test_orchestrator_resolves_models_from_config_yaml(
        ...
        # Mock 依赖以避免完整初始化
        with patch("coordinator.orchestrator.PlannerAgent"):
            with patch("coordinator.orchestrator.ReviewerAgent"):
                with patch("coordinator.orchestrator.WorkerPool"):
                    # 直接调用 _resolve_config_values 验证模型解析
                    orchestrator_cls = Orchestrator
                    # 使用 mock 避免完整初始化
                    resolved = orchestrator_cls._resolve_config_values(
                        orchestrator_cls, orch_config
                    )
...
class TestParseMaxIterations:
    ...
    def test_unlimited_keyword_max(self) -> None:
        """测试 MAX 关键词（无限迭代）"""
        from core.config import parse_max_iterations, UNLIMITED_ITERATIONS
    ...
    def test_invalid_string_raises_error(self) -> None:
        """测试非法字符串抛出异常"""
        from core.config import parse_max_iterations, MaxIterationsParseError
    ...
    def test_invalid_string_with_raise_on_error_false(self) -> None:
        """测试非法字符串不抛出异常时返回默认值"""
        from core.config import parse_max_iterations, DEFAULT_MAX_ITERATIONS
...
class TestNormalizeMaxIterations:
    ...
    def test_zero_becomes_unlimited(self) -> None:
        """测试零变为无限"""
        from core.config import normalize_max_iterations, UNLIMITED_ITERATIONS
...
class TestParseMaxIterationsForArgparse:
    ...
    def test_valid_values_work(self) -> None:
        """测试有效值正常工作"""
        from core.config import parse_max_iterations_for_argparse, UNLIMITED_ITERATIONS
...
class TestParseMaxIterationsConsistency:
    ...
    def test_run_py_uses_core_config_function(self) -> None:
        """验证 run.py 使用 core.config.parse_max_iterations"""
        import run
        from core.config import parse_max_iterations

        # run.py 应该从 core.config 导入 parse_max_iterations
        assert hasattr(run, 'parse_max_iterations') or 'parse_max_iterations' in dir(run)
...
class TestRunPyParseArgsTriState:
    ...
    def test_cloud_auth_timeout_tristate_not_specified_returns_none(
        ...
        assert args.cloud_auth_timeout is None, f"期望 cloud_auth_timeout=None（tri-state），实际 {args.cloud_auth_timeout}"
    ...
    def test_models_tristate_not_specified_returns_none(
        ...
        assert resolved["planner_model"] == "test-planner-model", f"期望 resolved planner_model='test-planner-model'，实际 {resolved['planner_model']}"
        assert resolved["worker_model"] == "test-worker-model", f"期望 resolved worker_model='test-worker-model'，实际 {resolved['worker_model']}"
        assert resolved["reviewer_model"] == "test-reviewer-model", f"期望 resolved reviewer_model='test-reviewer-model'，实际 {resolved['reviewer_model']}"
    ...
    def test_enable_sub_planners_tristate_not_specified_returns_none(
        ...
        assert args.enable_sub_planners is None, f"期望 enable_sub_planners=None（tri-state），实际 {args.enable_sub_planners}"
...
class TestRunMpPyParseArgsFromConfigYaml:
    ...
    def test_timeouts_from_config(
        ...
        assert args.execution_timeout is None, f"期望 execution_timeout=None（tri-state），实际 {args.execution_timeout}"
...
class TestRunPyMergeOptionsConfigToOrchestrator:
    ...
    def test_merge_options_uses_config_yaml_workers(
        ...
        from run import parse_args, Runner, TaskAnalysis
    ...
    def test_merge_options_uses_config_yaml_max_iterations(
        ...
        assert merged["max_iterations"] == 25, f"期望 max_iterations=25（来自 config.yaml），实际 {merged['max_iterations']}"
    ...
    def test_merge_options_uses_config_yaml_enable_sub_planners(
        ...
        assert merged["enable_sub_planners"] is False, f"期望 enable_sub_planners=False（来自 config.yaml），实际 {merged['enable_sub_planners']}"
    ...
    def test_merge_options_uses_config_yaml_execution_mode(
        ...
        assert merged["execution_mode"] == "auto", f"期望 execution_mode='auto'（来自 config.yaml），实际 {merged['execution_mode']}"
    ...
    def test_merge_options_uses_config_yaml_cloud_timeout(
        ...
        assert merged["cloud_timeout"] == 999, f"期望 cloud_timeout=999（来自 config.yaml），实际 {merged['cloud_timeout']}"
...
class TestRunPyIterateArgsConfigPropagation:
    ...
    def test_iterate_args_receives_config_yaml_values(
        ...
        assert merged["enable_sub_planners"] is True, f"期望 enable_sub_planners=True，实际 {merged['enable_sub_planners']}"
    ...
    def test_iterate_args_cli_overrides_config_yaml(
        ...
        assert merged["execution_mode"] == "cloud", f"期望 execution_mode='cloud'（CLI 覆盖），实际 {merged['execution_mode']}"

        # 未覆盖的应保持 config.yaml 值
        assert merged["enable_sub_planners"] is True, f"期望 enable_sub_planners=True（config.yaml），实际 {merged['enable_sub_planners']}"
...
class TestScriptParseArgsConsistency:
    ...
    def test_run_py_merge_options_default_matches_config(
        ...
        assert merged["execution_mode"] == "auto", f"期望 execution_mode='auto'（config.yaml），实际 {merged['execution_mode']}"
        ...
        assert merged["stream_log_console"] is False, f"期望 stream_log_console=False（config.yaml），实际 {merged['stream_log_console']}"
        assert merged["stream_log_detail_dir"] == "logs/test_stream/detail/", f"期望正确的 detail_dir"
        assert merged["stream_log_raw_dir"] == "logs/test_stream/raw/", f"期望正确的 raw_dir"
    ...
    def test_run_iterate_resolve_config_settings_matches_config(
        ...
        from scripts.run_iterate import parse_args, SelfIterator
        ...
        # 验证解析后的值与 config.yaml 一致
        assert settings.worker_pool_size == 5, f"期望 worker_pool_size=5（config.yaml），实际 {settings.worker_pool_size}"
        assert settings.max_iterations == 25, f"期望 max_iterations=25（config.yaml），实际 {settings.max_iterations}"
        assert settings.execution_mode == "auto", f"期望 execution_mode='auto'（config.yaml），实际 {settings.execution_mode}"
        assert settings.cloud_timeout == 600, f"期望 cloud_timeout=600（config.yaml），实际 {settings.cloud_timeout}"
        assert settings.stream_events_enabled is True, f"期望 stream_events_enabled=True（config.yaml），实际 {settings.stream_events_enabled}"
        assert settings.stream_log_console is False, f"期望 stream_log_console=False（config.yaml），实际 {settings.stream_log_console}"
...
class TestCloudAutoMpCompatibility:
    ...
    def test_run_py_cloud_mode_forces_basic_orchestrator(
        ...
        assert merged["orchestrator"] == "basic", (
            "Cloud 模式下 _merge_options 应将 orchestrator 强制切换为 basic"
        )
    ...
    def test_run_iterate_cloud_mode_forces_basic_orchestrator(
        ...
        assert orchestrator_type == "basic", (
            f"Cloud 模式应强制使用 basic 编排器，实际返回 {orchestrator_type}"
        )
    ...
    def test_run_iterate_auto_mode_forces_basic_orchestrator(
        ...
        assert orchestrator_type == "basic", (
            f"Auto 模式应强制使用 basic 编排器，实际返回 {orchestrator_type}"
        )
    ...
    def test_run_iterate_cli_mode_allows_mp_orchestrator(
        ...
        assert orchestrator_type == "mp", (
            f"CLI 模式应允许使用 MP 编排器，实际返回 {orchestrator_type}"
        )

    def test_run_iterate_no_mp_flag_forces_basic(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
        ...
        assert orchestrator_type == "basic", (
            f"--no-mp 应强制使用 basic 编排器，实际返回 {orchestrator_type}"
        )
    ...
    def test_resolve_orchestrator_settings_cli_allows_mp(
        ...
        assert settings["orchestrator"] == "mp", (
            f"CLI 模式应允许 orchestrator 为 mp，实际 {settings['orchestrator']}"
        )
    ...
    def test_ampersand_prefix_triggers_cloud_mode(
        ...
        # 验证 & 前缀被检测
        assert iterator._is_cloud_request is True, "应检测到 & 前缀"

        # 验证执行模式为 CLOUD
        execution_mode = iterator._get_execution_mode()
        assert execution_mode == ExecutionMode.CLOUD, (
            f"& 前缀应触发 CLOUD 模式，实际 {execution_mode}"
        )

        # 验证编排器被强制为 basic
        orchestrator_type = iterator._get_orchestrator_type()
        assert orchestrator_type == "basic", (
            f"& 前缀触发 CLOUD 模式应强制使用 basic 编排器，实际 {orchestrator_type}"
        )
    ...
    def test_consistency_between_run_py_and_run_iterate_py(
        ...
        from run import parse_args as run_parse_args, Runner, TaskAnalysis
...
class TestRunPyModelParamsFromConfigYaml:
    ...
    def test_cli_model_params_override_config_yaml(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
        ...
        monkeypatch.setattr(sys, "argv", [
            "run.py",
            "--planner-model", "cli-planner-model",
            "--worker-model", "cli-worker-model",
            "--reviewer-model", "cli-reviewer-model",
            "测试任务",
        ])
    ...
    def test_model_params_tri_state_none_means_use_config(
        ...
        assert args.planner_model is None, (
            "未传递 --planner-model 时 args.planner_model 应为 None"
        )
        assert args.worker_model is None, (
            "未传递 --worker-model 时 args.worker_model 应为 None"
        )
        assert args.reviewer_model is None, (
            "未传递 --reviewer-model 时 args.reviewer_model 应为 None"
        )
...
class TestEntryScriptConfigConsistency:
    ...
    def test_core_config_consistency_without_cli_overrides(
        ...
        assert run_merged["worker_model"] == iterate_settings.worker_model, (
            f"worker_model 不一致: run.py={run_merged['worker_model']}, "
            f"run_iterate.py={iterate_settings.worker_model}"
        )
    ...
    def test_core_config_consistency_with_cli_overrides(
        ...
        monkeypatch.setattr(sys, "argv", [
            ...
            "--max-iterations", str(cli_max_iterations),
        ...
        assert run_merged["max_iterations"] == cli_max_iterations, (
            f"run.py max_iterations 未正确覆盖: expected={cli_max_iterations}, "
            f"actual={run_merged['max_iterations']}"
        )
        ...
        assert run_merged["cloud_timeout"] == cli_cloud_timeout, (
            f"run.py cloud_timeout 未正确覆盖: expected={cli_cloud_timeout}, "
            f"actual={run_merged['cloud_timeout']}"
        )
    ...
    def test_orchestrator_settings_with_overrides(
        ...
        assert resolved["orchestrator"] == "basic", (
            "execution_mode=cloud 时 orchestrator 应自动切换为 basic"
        )
...
"""
    ...
    def test_auto_mode_forces_basic_orchestrator(
        ...
        assert resolved["execution_mode"] == "auto", (
            f"execution_mode 应为 auto，实际 {resolved['execution_mode']}"
        )
        assert resolved["orchestrator"] == "basic", (
            f"execution_mode=auto 时 orchestrator 应自动切换为 basic，实际 {resolved['orchestrator']}"
        )
        assert resolved["cloud_timeout"] == 900, (
            f"cloud_timeout 应为 900，实际 {resolved['cloud_timeout']}"
        )
        assert resolved["cloud_auth_timeout"] == 60, (
            f"cloud_auth_timeout 应为 60，实际 {resolved['cloud_auth_timeout']}"
        )

    def test_cli_mode_keeps_mp_orchestrator(
        self, tmp_path: Path, reset_config_manager, monkeypatch
    ) -> None:
...
"""
        ...
        assert resolved["execution_mode"] == "cli", (
            f"execution_mode 应为 cli，实际 {resolved['execution_mode']}"
        )
...
"""
        ...
        assert resolved["execution_mode"] == "cloud", (
            "CLI 覆盖后 execution_mode 应为 cloud"
        )
        assert resolved["orchestrator"] == "basic", (
            "execution_mode=cloud（CLI 覆盖）时 orchestrator 应切换为 basic"
        )
    ...
    def test_cloud_timeout_passthrough_in_run_mp_context(
        ...
        from core.config import resolve_orchestrator_settings, get_config
...
"""
    ...
    def test_iterate_resolves_cloud_config_from_yaml(
        ...
        assert settings.execution_mode == "auto", (
            f"execution_mode 应为 auto，实际 {settings.execution_mode}"
        )
        assert settings.cloud_timeout == 750, (
            f"cloud_timeout 应为 750，实际 {settings.cloud_timeout}"
        )
        assert settings.cloud_auth_timeout == 55, (
            f"cloud_auth_timeout 应为 55，实际 {settings.cloud_auth_timeout}"
        )
    ...
    def test_iterate_cli_overrides_cloud_config(
        ...
        assert settings.cloud_timeout == 1500, (
            f"CLI 覆盖后 cloud_timeout 应为 1500，实际 {settings.cloud_timeout}"
        )
...
"""
    ...
    def test_knowledge_parses_cloud_config(
        ...
        assert args.execution_mode == "cloud", (
            f"期望 execution_mode='cloud'，实际 {args.execution_mode}"
        )
        assert args.cloud_timeout == 800, (
            f"期望 cloud_timeout=800，实际 {args.cloud_timeout}"
        )
...
class TestStreamConfigTriState:
    ...
    def test_default_stream_constants_match_config_yaml(
        ...
        # 验证常量值与代码注释一致
        assert DEFAULT_STREAM_EVENTS_ENABLED is False, (
            "DEFAULT_STREAM_EVENTS_ENABLED 应为 False（与 config.yaml 同步）"
        )
        assert DEFAULT_STREAM_LOG_CONSOLE is True, (
            "DEFAULT_STREAM_LOG_CONSOLE 应为 True（与 config.yaml 同步）"
        )
        assert DEFAULT_STREAM_LOG_DETAIL_DIR == "logs/stream_json/detail/", (
            "DEFAULT_STREAM_LOG_DETAIL_DIR 应与 config.yaml 同步"
        )
        assert DEFAULT_STREAM_LOG_RAW_DIR == "logs/stream_json/raw/", (
            "DEFAULT_STREAM_LOG_RAW_DIR 应与 config.yaml 同步"
        )
    ...
    def test_cursor_agent_config_stream_default_matches_config_yaml(
        ...
        assert config.stream_events_enabled is False, (
            "CursorAgentConfig.stream_events_enabled 默认应为 False（与 config.yaml 同步），"
            "避免'未注入即开启'的问题"
        )
    ...
    def test_mp_orchestrator_resolves_stream_from_config_yaml_when_none(
        ...
        # 验证解析后的值来自 custom_config.yaml
        assert resolved["stream_events_enabled"] is True, (
            "stream_events_enabled 应从 config.yaml 读取（enabled=True）"
        )
        assert resolved["stream_log_console"] is False, (
            "stream_log_console 应从 config.yaml 读取（console=False）"
        )
    ...
    def test_mp_orchestrator_uses_explicit_stream_over_config_yaml(
        ...
        # 验证显式值优先
        assert resolved["stream_events_enabled"] is False, (
            "显式传入的 stream_events_enabled=False 应覆盖 config.yaml"
        )
        assert resolved["stream_log_console"] is True, (
            "显式传入的 stream_log_console=True 应覆盖 config.yaml"
        )
...
class TestRunBasicPyConfigPrecedence:
    ...
    def test_run_basic_parse_args_defaults(
        ...
        # 动态导入 scripts/run_basic.py
        spec = importlib.util.spec_from_file_location(
            "run_basic", "scripts/run_basic.py"
        )
...
class TestRunMpPyConfigPrecedence:
    ...
    def test_run_mp_parse_args_defaults(
        ...
        spec = importlib.util.spec_from_file_location("run_mp", "scripts/run_mp.py")
...
class TestRunIteratePyConfigPrecedence:
    ...
    def test_run_iterate_parse_args_defaults(
        ...
        spec = importlib.util.spec_from_file_location(
            "run_iterate", "scripts/run_iterate.py"
        )
        ...
        # orchestrator 默认为 "mp"（不是 tri-state）
...
class TestRunKnowledgePyConfigPrecedence:
    ...
    def test_run_knowledge_parse_args_defaults(
        ...
        spec = importlib.util.spec_from_file_location(
            "run_knowledge", "scripts/run_knowledge.py"
        )
...
class TestStreamEventsEnabledEndToEnd:
    ...
    def test_stream_enabled_flows_to_cursor_agent_config(
        ...
        agent_config_dict = build_cursor_agent_config(
            working_directory=str(tmp_path), overrides=None
        )
        assert agent_config_dict["stream_events_enabled"] is True

        # 步骤 3: 验证 CursorAgentConfig 实例正确接收该值
        agent_config = CursorAgentConfig(
            stream_events_enabled=agent_config_dict["stream_events_enabled"]
        )
        assert agent_config.stream_events_enabled is True
...
class TestPrintConfigDebugOutput:
    ...
    def test_format_debug_config_output_structure(
        ...
        from core.config import format_debug_config, CONFIG_DEBUG_PREFIX
        ...
        assert f"{CONFIG_DEBUG_PREFIX} execution_mode: cli" in output
    ...
    def test_format_debug_config_orchestrator_fallback_cloud(
        ...
        assert f"{CONFIG_DEBUG_PREFIX} execution_mode: cloud" in output
        assert "mp->basic" in output
        assert "execution_mode=cloud" in output
    ...
    def test_format_debug_config_orchestrator_fallback_auto(
        ...
        # 验证回退信息
        assert f"{CONFIG_DEBUG_PREFIX} orchestrator: basic" in output
        assert f"{CONFIG_DEBUG_PREFIX} execution_mode: auto" in output
        assert "mp->basic" in output
        assert "execution_mode=auto" in output
    ...
    def test_print_debug_config_function(
        ...
        from core.config import print_debug_config, CONFIG_DEBUG_PREFIX
...
class TestEntryScriptConfigLoadingComplete:
    ...
    def test_run_py_config_from_yaml_no_cli_args(
        ...
        assert merged["max_iterations"] == 15, (
            f"max_iterations 应为 15, 实际 {merged['max_iterations']}"
        )
        assert merged["execution_mode"] == "cli", (
            f"execution_mode 应为 cli, 实际 {merged['execution_mode']}"
        )
        assert merged["cloud_timeout"] == 450, (
            f"cloud_timeout 应为 450, 实际 {merged['cloud_timeout']}"
        )
        assert merged["stream_log"] is True, (
            f"stream_log 应为 True, 实际 {merged['stream_log']}"
        )
    ...
    def test_run_py_cli_overrides_config(
        ...
        assert merged["max_iterations"] == 8, (
            f"CLI max_iterations 应覆盖为 8, 实际 {merged['max_iterations']}"
        )
        assert merged["execution_mode"] == "cloud", (
            f"CLI execution_mode 应覆盖为 cloud, 实际 {merged['execution_mode']}"
        )
        assert merged["cloud_timeout"] == 200, (
            f"CLI cloud_timeout 应覆盖为 200, 实际 {merged['cloud_timeout']}"
        )
        assert merged["stream_log"] is False, (
            f"CLI --no-stream-log 应禁用 stream_log, 实际 {merged['stream_log']}"
        )
    ...
    def test_run_py_cloud_mode_forces_basic_orchestrator(
        ...
        settings = resolve_orchestrator_settings(overrides=None)
        assert settings["orchestrator"] == "basic", (
            f"cloud 模式应强制 basic 编排器, 实际 {settings['orchestrator']}"
        )
        assert settings["execution_mode"] == "cloud"
    ...
    def test_run_py_auto_mode_forces_basic_orchestrator(
        ...
        settings = resolve_orchestrator_settings(overrides=None)
        assert settings["orchestrator"] == "basic", (
            f"auto 模式应强制 basic 编排器, 实际 {settings['orchestrator']}"
        )
        assert settings["execution_mode"] == "auto"
    ...
    def test_run_basic_py_config_from_yaml_no_cli_args(
        ...
        assert args.workers is None, (
            f"无 CLI 参数时 workers 应为 None, 实际 {args.workers}"
        )
        assert args.max_iterations is None, (
            f"无 CLI 参数时 max_iterations 应为 None, 实际 {args.max_iterations}"
        )
        assert args.stream_log_enabled is None, (
        ...
        from core.config import resolve_orchestrator_settings, build_cursor_agent_config
        ...
        agent_config = build_cursor_agent_config(
            working_directory=str(tmp_path), overrides=None
        )
        assert agent_config["stream_events_enabled"] is True
    ...
    def test_run_basic_py_cli_overrides_config(
        ...
        assert args.max_iterations == "12", (
            f"CLI max_iterations 应为 '12', 实际 {args.max_iterations}"
        )
        assert args.stream_log_enabled is False, (
            f"CLI --no-stream-log 应为 False, 实际 {args.stream_log_enabled}"
        )
    ...
    def test_run_mp_py_cloud_mode_forces_basic_fallback(
        ...
        settings = resolve_orchestrator_settings(overrides=None)
        assert settings["orchestrator"] == "basic", (
            f"cloud 模式应强制 basic, 实际 {settings['orchestrator']}"
        )
        assert settings["execution_mode"] == "cloud"
    ...
    def test_run_mp_py_auto_mode_forces_basic_fallback(
        ...
        settings = resolve_orchestrator_settings(overrides=None)
        assert settings["orchestrator"] == "basic", (
            f"auto 模式应强制 basic, 实际 {settings['orchestrator']}"
        )
        assert settings["execution_mode"] == "auto"
    ...
    def test_run_iterate_py_config_from_yaml_no_cli_args(
        ...
        # 验证配置值来自 config.yaml
        assert settings.worker_pool_size == 7, (
            f"worker_pool_size 应为 7, 实际 {settings.worker_pool_size}"
        )
        assert settings.max_iterations == 15, (
            f"max_iterations 应为 15, 实际 {settings.max_iterations}"
        )
        assert settings.execution_mode == "cli", (
            f"execution_mode 应为 cli, 实际 {settings.execution_mode}"
        )
        assert settings.cloud_timeout == 450, (
            f"cloud_timeout 应为 450, 实际 {settings.cloud_timeout}"
        )
        assert settings.stream_events_enabled is True, (
    ...
    def test_run_iterate_py_cli_overrides_config(
        ...
        assert settings.worker_pool_size == 2, (
            f"CLI workers 应覆盖为 2, 实际 {settings.worker_pool_size}"
        )
        assert settings.max_iterations == 5, (
            f"CLI max_iterations 应覆盖为 5, 实际 {settings.max_iterations}"
        )
        assert settings.cloud_timeout == 100, (
            f"CLI cloud_timeout 应覆盖为 100, 实际 {settings.cloud_timeout}"
        )
    ...
    def test_run_iterate_py_cloud_mode_forces_basic_orchestrator(
        ...
        orchestrator_type = iterator._get_orchestrator_type()
        assert orchestrator_type == "basic", (
            f"cloud 模式应强制 basic 编排器, 实际 {orchestrator_type}"
        )
    ...
    def test_run_iterate_py_auto_mode_forces_basic_orchestrator(
        ...
        orchestrator_type = iterator._get_orchestrator_type()
        assert orchestrator_type == "basic", (
            f"auto 模式应强制 basic 编排器, 实际 {orchestrator_type}"
        )
    ...
    def test_run_iterate_py_stream_events_enabled_from_yaml(
        ...
        config = CursorAgentConfig(
            stream_events_enabled=iterator._resolved_settings.stream_events_enabled
        )
    ...
    def test_all_scripts_consistent_config_resolution(
        ...
        for mod_name in list(sys.modules.keys()):
            if mod_name.startswith("run") or mod_name.startswith("scripts."):
                del sys.modules[mod_name]
        from run import parse_args as run_parse, Runner
        ...
        for mod_name in list(sys.modules.keys()):
            if "run_iterate" in mod_name or mod_name.startswith("scripts."):
                del sys.modules[mod_name]
        from scripts.run_iterate import parse_args as iterate_parse, SelfIterator
        ...
        for script_name, values in results.items():
            assert values["workers"] == expected_workers, (
                f"{script_name}: workers 应为 {expected_workers}, "
                f"实际 {values['workers']}"
            )
            assert values["max_iterations"] == expected_max_iterations, (
                f"{script_name}: max_iterations 应为 {expected_max_iterations}, "
                f"实际 {values['max_iterations']}"
            )
    ...
    def test_all_scripts_cli_override_precedence(
        ...
        ConfigManager.reset_instance()
        monkeypatch.setattr(
            sys, "argv", ["run.py", "--workers", str(cli_workers), "测试任务"]
        )
        for mod_name in list(sys.modules.keys()):
            if mod_name.startswith("run") or mod_name.startswith("scripts."):
                del sys.modules[mod_name]
        ...
        for script_name, workers in results.items():
            assert workers == cli_workers, (
                f"{script_name}: CLI --workers {cli_workers} 应覆盖配置, "
                f"实际 {workers}"
            )
