...
class TestNoApiKeyNetworkIsolation:
    ...
    def test_cloud_executor_not_called_without_api_key(self):
        ...
        """
        from cursor.cloud.auth import AuthStatus, CloudAuthManager
    ...
    def test_classify_cloud_failure_no_key(self):
        ...
        for msg in no_key_messages:
            ...
            assert failure_info.retryable is False, f"NO_KEY 错误不应可重试，消息: {msg}"
    ...
    def test_build_execution_decision_no_api_key_fallback(self):
        ...
        assert decision.effective_mode == "cli", f"无 API Key 时 auto 模式应回退到 CLI，实际 {decision.effective_mode}"
        # requested_mode=auto 时编排器仍强制 basic
        assert decision.orchestrator == "basic", f"requested_mode=auto 时编排器应为 basic，实际 {decision.orchestrator}"
        ...
        assert decision.prefix_routed is False, "无 API Key 时 & 前缀不应成功路由"
...
class TestClassifyCloudFailureNoKeyIntegration:
    ...
    def test_classify_various_no_key_patterns(self):
        ...
        test_cases: list[tuple[str | dict[str, str], CloudFailureKind]] = [
            # (输入, 预期 kind)
        ...
        for input_val, expected_kind in test_cases:
            ...
            assert result.kind == expected_kind, f"输入 {input_val!r} 应分类为 {expected_kind}，实际 {result.kind}"
    ...
    def test_classify_auth_vs_no_key(self):
        ...
        for msg in auth_patterns:
            ...
            assert result.kind == CloudFailureKind.AUTH, f"消息 '{msg}' 应分类为 AUTH"
        ...
        for msg in no_key_patterns:
            result = classify_cloud_failure(msg)
            assert result.kind == CloudFailureKind.NO_KEY, f"消息 '{msg}' 应分类为 NO_KEY"
