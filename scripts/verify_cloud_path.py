...
def verify_cloud_utils():
    ...
    try:
        ...
        strip_passed = True
        for prompt, expected_prefix in strip_cases:
            strip_result = strip_cloud_prefix(prompt)
            if strip_result != expected_prefix:
                strip_passed = False
...
def verify_cloud_client_factory():
    ...
    try:
        ...
        print_check("resolve_api_key()", True, f"{'已配置 API Key' if has_key else '未配置 API Key'}")
...
def verify_executor():
    ...
    try:
        ...
        print_check("ExecutionMode 枚举", set(modes) == {"cli", "cloud", "auto", "plan", "ask"}, f"模式: {modes}")
...
def verify_execution_policy():
    ...
    try:
        ...
        print_check("CloudFailureKind 枚举", "auth" in kinds and "rate_limit" in kinds, f"类型: {kinds}")
        ...
        print_check("resolve_effective_execution_mode()", mode == "cloud", f"模式: {mode}, 原因: {reason}")
        ...
        print_check("classify_cloud_failure()", info.kind == CloudFailureKind.AUTH, f"分类结果: {info.kind.value}")
